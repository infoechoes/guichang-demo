(() => {
 ...
 function switchSource(){epoch++;result=null;$('#results').replaceChildren();$('#export').disabled=true;$('#submit').disabled=false;const market=$('#platform').value==='xinfadi';$('#results').style.display=market?'block':'';$('#query-label').textContent=market?'食材名称':'商品名称 / 型号';$('#market-date-label').hidden=!market;$('#market-date-label').style.display=market?'':'none';$('#query').placeholder=market?'例如：大白菜、土豆':'例如：惠普 915XL 黑色 原装';$('#query').maxLength=market?80:120;$('#status').textContent=market?'输入食材品名；日期留空查近 7 天最新可用行情。':'输入商品信息开始查询。';$('.notice').textContent=market?'新发地市场参考行情，不是供应商报价。保留原单位，不自动把斤、公斤、箱换算；同名食材仍需核对规格和产地。':'参考价不等于最终报价，请确认规格、优惠条件和运费。';$('.examples').style.display=market?'none':'';}
